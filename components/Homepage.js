...
export default function Homepage({ navigation }) {
  ...
  const onChange = (selectedDate) => {
    ...
    let eventsSelectedDay = listItems
      .filter((event) => event.event_dates.starting_day >= selectedMorning)
      .filter((event) => event.event_dates.starting_day <= selectedNight);
  ...
  function getEventsToday() {
    let nextMidnight = moment(
      moment().format("YYYY-MM-DD") + " 23:59:00"
    ).toISOString();

    let momentTime = moment(moment().format()).toISOString();

    let eventsToday = listItems
      .filter((event) => event.event_dates.starting_day >= momentTime)
      .filter((event) => event.event_dates.starting_day <= nextMidnight);
  ...
  function getEventsTomorrow() {
    ...
    let eventsTomorrow = listItems
      .filter((event) => event.event_dates.starting_day >= tomorrowMorning)
      .filter((event) => event.event_dates.starting_day <= tomorrowNight);
      ...
      <View style={styles.HomepageContainer}>
        ...
        <View style={styles.Icon}>
          <Icon
            ...
            name="search"
            onPress={() =>
              navigation.navigate("Eventlist", { data: listItems })
            }
          />
        </View>
